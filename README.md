...
**通知（仅私聊）**：`通知` 可分别开关课表 / 作业 / 第二课堂 / 待办提醒；课前约 30 分钟、未完成作业／待办截止前 24 小时内推送图卡，第二课堂按订阅事件推送。待办默认关闭，发送 `通知 待办 开` 启用；无截止时间、已完成和已过期的待办不提醒。群聊不推个人通知。
...
原始 `conversation_events` 不删除。部署脚本在停机备份后运行 `migrate`，显式创建该表、添加默认关闭的第二课堂和待办提醒设置列，并
