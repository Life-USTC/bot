...
// CapabilityAudience describes where an invocation may run. GroupReadOnly is
// still gated by Handler.AllowGroupPersonalInfo; Any is available in groups.
type CapabilityAudience string

const (
	AudienceAny           CapabilityAudience = "any"
	AudiencePrivate       CapabilityAudience = "private"
	AudienceGroupReadOnly CapabilityAudience = "group_read_only"
)
...
func (d CapabilityDescriptor) PolicyFor(inv Invocation) CapabilityPolicy {
	policy := CapabilityPolicy{
		Effect:       d.Effect,
		Audience:     d.Requirements.Audience,
		Exposure:     d.Exposure,
...
func policyFor(inv Invocation, effect CapabilityEffect, audience CapabilityAudience, exposure ResultExposure, confirmation ConfirmationPolicy) CapabilityPolicy {
	return CapabilityPolicy{Effect: effect, Audience: audience, Exposure: exposure, Confirmation: confirmation}
}

func readPolicy(inv Invocation, audience CapabilityAudience) CapabilityPolicy {
	return policyFor(inv, EffectRead, audience, ExposureModel, ConfirmNever)
}

func privateWritePolicy(inv Invocation, effect CapabilityEffect) CapabilityPolicy {
	return policyFor(inv, effect, AudiencePrivate, ExposureModel, ConfirmUser)
}

func helpPolicy(inv Invocation) CapabilityPolicy {
	if len(inv.Args) == 0 {
		return readPolicy(inv, AudienceAny)
	}
	return readPolicy(inv, AudienceGroupReadOnly)
}

func loginPolicy(inv Invocation) CapabilityPolicy {
	return policyFor(inv, EffectWrite, AudiencePrivate, ExposureHostOnly, ConfirmNever)
}

func subscriptionPolicy(inv Invocation) CapabilityPolicy {
	if firstArgIs(inv.Args, "link") {
		return policyFor(inv, EffectRead, AudiencePrivate, ExposureHostOnly, ConfirmNever)
	}
...
func feedbackPolicy(inv Invocation) CapabilityPolicy {
	if hasArgs(inv.Args) && !firstArgIs(inv.Args, "help") {
		return policyFor(inv, EffectWrite, AudienceAny, ExposureModel, ConfirmUser)
	}
	return readPolicy(inv, AudienceAny)
}
...
func init() {
	capabilityDescriptors = []CapabilityDescriptor{
		descriptor(CapabilityHelp, []string{"help", "帮助", "菜单"}, CapabilityRequirements{Audience: AudienceAny}, EffectRead, ExposureModel, allowArgs, nil, nil, helpPolicy, helpMeta("help", "帮助", "查看 Bot 命令总览与专题用法", false, nil, nil)),
		descriptor(CapabilityLogin, []string{"login", "登录"}, CapabilityRequirements{OAuth: true, Audience: AudiencePrivate}, EffectWrite, ExposureHostOnly, func(args []string) bool { return !hasArgs(args) || firstArgIsHelp(args) || firstArgIs(args, "status") }, normalizeLoginArgs, func(h Handler, ctx context.Context, ident store.Identity, args []string) string {
		...
		descriptor(CapabilityLogout, []string{"logout", "退出"}, CapabilityRequirements{OAuth: true, Audience: AudiencePrivate}, EffectDestructive, ExposureModel, func(args []string) bool { return !hasArgs(args) || firstArgIsHelp(args) }, nil, func(h Handler, ctx context.Context, ident store.Identity, _ []string) string {
		...
		descriptor(CapabilityAccount, []string{"account", "账户", "我的"}, CapabilityRequirements{Life: true, OAuth: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, func(args []string) bool { return !hasArgs(args) || firstArgIsHelp(args) }, nil, func(h Handler, ctx context.Context, ident store.Identity, _ []string) string { return h.me(ctx, ident) }, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("account", "账户", "登录、退出与查看账户信息", false, []HelpExample{example("账户 信息", "查看当前登录用户")}, []HelpExample{example("我的", "相当于“账户 信息")})),
		descriptor(CapabilityTodo, []string{"todo", "待办", "td"}, CapabilityRequirements{Life: true, OAuth: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, allowArgs, normalizeTodoArgs, func(h Handler, ctx context.Context, ident store.Identity, args []string) string {
		...
		descriptor(CapabilityHomework, []string{"homework", "作业", "hw"}, CapabilityRequirements{Life: true, OAuth: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, homeworkOrTodoInput(homeworkArgsAcceptable), normalizeHomeworkArgs, func(h Handler, ctx context.Context, ident store.Identity, args []string) string {
		...
		descriptor(CapabilityCalendar, []string{"calendar", "日程", "今日", "ddl"}, CapabilityRequirements{Life: true, OAuth: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, noArgsOrHelp, nil, func(h Handler, ctx context.Context, ident store.Identity, _ []string) string {
			return h.overview(ctx, ident)
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("agenda", "日程", "今日安排、综合概览与近期截止", true, []HelpExample{example("日程 今日", "汇总今日课程、待办和作业")}, []HelpExample{example("今日（ddl）", "相当于“日程 今日")})),
		descriptor(CapabilitySubscription, []string{"subscription", "订阅", "课程订阅"}, CapabilityRequirements{Life: true, OAuth: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, subscriptionArgsAcceptable, normalizeSubscriptionArgs, func(h Handler, ctx context.Context, ident store.Identity, args []string) string {
		...
		descriptor(CapabilityNotify, []string{"notify", "通知", "提醒"}, CapabilityRequirements{Store: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, notifyArgsAcceptable, normalizeNotifyArgs, func(h Handler, ctx context.Context, ident store.Identity, args []string) string {
		...
		descriptor(CapabilitySettings, []string{"settings", "设置"}, CapabilityRequirements{Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, settingsArgsAcceptable, nil, func(h Handler, ctx context.Context, ident store.Identity, args []string) string {
			return h.settings(ctx, ident, args)
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("settings", "设置", "管理通知等偏好", true, []HelpExample{example("设置", "查看设置命令")}, nil)),
		descriptor(CapabilityAgentSettings, []string{"agent", "AI", "AI工具"}, CapabilityRequirements{Store: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, agentArgsAcceptable, normalizeAgentArgs, func(h Handler, ctx context.Context, ident store.Identity, args []string) string {
		...
		}, feedbackPolicy, helpMeta("feedback", "反馈", "向管理员提交反馈", true, []HelpExample{exampleFor(CapabilityFeedback, "反馈 <你的建议>", "向管理员提交反馈", "<你的建议>")}, nil)),
		...
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudiencePrivate) }, helpMeta("system", "系统", "查看服务状态与检查连通性", false, []HelpExample{example("系统 检查", "检查 Life @ USTC API 是否可用")}, nil)),
		descriptor(CapabilityStatus, []string{"status", "状态"}, CapabilityRequirements{Life: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, noArgsOrHelp, nil, func(h Handler, ctx context.Context, ident store.Identity, _ []string) string {
			return h.status(ctx, ident)
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("system", "系统", "查看服务状态与检查连通性", true, []HelpExample{example("系统 状态", "查看服务与登录状态")}, []HelpExample{example("状态（status）", "相当于“系统 状态")})),
		descriptor(CapabilitySemester, []string{"semester", "学期"}, CapabilityRequirements{Life: true, Audience: AudienceGroupReadOnly, PublicCache: true}, EffectRead, ExposureModel, semesterInput, nil, func(h Handler, ctx context.Context, _ store.Identity, _ []string) string {
			return h.currentSemester(ctx)
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("semester", "学期", "查看当前学期和学期列表", false, []HelpExample{example("学期", "查看当前学期"), example("学期 当前", "查看当前学期")}, nil)),
		descriptor(CapabilityCourse, []string{"course", "课程"}, CapabilityRequirements{Life: true, Audience: AudienceGroupReadOnly, PublicCache: true}, EffectRead, ExposureModel, allowArgs, nil, func(h Handler, ctx context.Context, _ store.Identity, args []string) string {
			return h.searchCourses(ctx, joinedArgs(args))
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("course", "课程", "搜索课程和查看课程详情", false, []HelpExample{example("课程 数学分析", "按关键词快速搜索课程")}, nil)),
		descriptor(CapabilitySection, []string{"section", "教学班"}, CapabilityRequirements{Life: true, Audience: AudienceGroupReadOnly, PublicCache: true}, EffectRead, ExposureModel, allowArgs, nil, func(h Handler, ctx context.Context, _ store.Identity, args []string) string {
			return h.searchSections(ctx, joinedArgs(args))
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("section", "教学班", "搜索教学班及其课表、考试、作业", false, []HelpExample{example("教学班 高等数学", "按关键词快速搜索教学班")}, nil)),
		descriptor(CapabilityTeacher, []string{"teacher", "老师"}, CapabilityRequirements{Life: true, Audience: AudienceGroupReadOnly, PublicCache: true}, EffectRead, ExposureModel, allowArgs, nil, func(h Handler, ctx context.Context, _ store.Identity, args []string) string {
			return h.searchTeachers(ctx, joinedArgs(args))
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("teacher", "老师", "搜索老师和查看老师详情", true, []HelpExample{example("老师 张", "按关键词快速搜索老师")}, nil)),
		descriptor(CapabilityBus, []string{"bus", "校车", "xc"}, CapabilityRequirements{Life: true, Audience: AudienceAny}, EffectRead, ExposureModel, busCommandArgsAcceptable, nil, func(h Handler, ctx context.Context, ident store.Identity, args []string) string {
		...
		descriptor(CapabilitySchedule, []string{"schedule", "课表", "kb"}, CapabilityRequirements{Life: true, OAuth: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, scheduleArgsAcceptable, normalizeScheduleArgs, func(h Handler, ctx context.Context, ident store.Identity, args []string) string {
			return h.curriculum(ctx, ident, args)
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("schedule", "课表", "周课表、单日课表与下一节课", true, []HelpExample{example("课表", "查看本周课表"), example("课表 本周", "查看本周课表"), example("课表 下周", "查看下周课表"), example("课表 第3周", "查看指定教学周"), example("课表 2026 秋季学期", "查看整学期课表与教学周范围"), example("课表 单日 今天", "只查看今天的课表"), example("课表 单日 明天", "只查看明天的课表")}, []HelpExample{example("今日课表（单日课表）", "相当于“课表 单日 今天")})),
		descriptor(CapabilityNextClass, []string{"nextclass", "下一节课"}, CapabilityRequirements{Life: true, OAuth: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, noArgsOrHelp, nil, func(h Handler, ctx context.Context, ident store.Identity, _ []string) string {
			return h.nextClass(ctx, ident)
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("schedule", "课表", "周课表、单日课表与下一节课", false, []HelpExample{example("课表 下一节", "查看最近一节课")}, []HelpExample{example("下一节课", "相当于“课表 下一节")})),
		descriptor(CapabilityExam, []string{"exam", "考试", "ks"}, CapabilityRequirements{Life: true, OAuth: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, examArgsAcceptable, nil, func(h Handler, ctx context.Context, ident store.Identity, args []string) string {
			return h.exams(ctx, ident, args)
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("exam", "考试", "查看已订阅课程的考试", true, []HelpExample{example("考试", "查看已订阅课程的考试，每页 30 条"), example("考试 第2页", "查看考试的第 2 页")}, nil)),
		descriptor(CapabilityListSemesters, []string{"list_semesters", "学期列表"}, CapabilityRequirements{Life: true, Audience: AudienceGroupReadOnly, PublicCache: true}, EffectRead, ExposureModel, semesterListInput, nil, func(h Handler, ctx context.Context, _ store.Identity, args []string) string {
		...
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("course", "课程", "搜索课程和查看课程详情", false, []HelpExample{example("课程 搜索 数学分析", "搜索课程"), exampleFor(CapabilityCourseSearch, "课程 搜索 培养层次ID <ID>", "按培养层次筛选", "education_level_id", "<ID>"), exampleFor(CapabilityCourseSearch, "课程 搜索 类别ID <ID>", "按课程类别筛选", "category_id", "<ID>"), exampleFor(CapabilityCourseByJWID, "课程 查看 <JW ID>", "按 JW ID 查看详情", "<JW ID>")}, nil)),
		...
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("section", "教学班", "搜索教学班及其课表、考试、作业", false, []HelpExample{example("教学班 搜索 高等数学", "搜索教学班"), exampleFor(CapabilitySectionByJWID, "教学班 查看 <JW ID>", "按 JW ID 查看详情", "<JW ID>")}, nil)),
		...
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("teacher", "老师", "搜索老师和查看老师详情", false, []HelpExample{example("老师 搜索 张", "搜索老师"), exampleFor(CapabilityTeacherByID, "老师 查看 <ID>", "按 ID 查看详情", "<ID>")}, nil)),
		...
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("course", "课程", "搜索课程和查看课程详情", false, []HelpExample{exampleFor(CapabilityCourseByJWID, "课程 查看 <JW ID>", "按 JW ID 查看详情", "<JW ID>")}, nil)),
		...
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("section", "教学班", "搜索教学班及其课表、考试、作业", false, []HelpExample{exampleFor(CapabilitySectionByJWID, "教学班 查看 <JW ID>", "按 JW ID 查看详情", "<JW ID>")}, nil)),
		...
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("teacher", "老师", "搜索老师和查看老师详情", false, []HelpExample{exampleFor(CapabilityTeacherByID, "老师 查看 <ID>", "按 ID 查看详情", "<ID>")}, nil)),
		...
		}, func(inv Invocation) CapabilityPolicy { return privateWritePolicy(inv, EffectDestructive) }, helpMeta("subscription", "订阅", "查看、导入和管理教学班订阅", false, []HelpExample{exampleFor(CapabilityUnsubscribeSectionByJWID, "订阅 删除 <JW ID>", "按 JW ID 退订教学班", "<JW ID>")}, nil)),
		...
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("section", "教学班", "搜索教学班及其课表、考试、作业", false, []HelpExample{exampleFor(CapabilitySectionSchedules, "教学班 课表 <JW ID> <开始日期> <结束日期>", "查看日期范围内的课表", "<JW ID>", "<开始日期>", "<结束日期>")}, nil)),
		...
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("section", "教学班", "搜索教学班及其课表、考试、作业", false, []HelpExample{exampleFor(CapabilitySectionExams, "教学班 考试 <JW ID>", "查看指定教学班考试，每页 30 条", "<JW ID>")}, nil)),
		...
		}, func(inv Invocation) CapabilityPolicy { return readPolicy(inv, AudienceGroupReadOnly) }, helpMeta("section", "教学班", "搜索教学班及其课表、考试、作业", false, []HelpExample{exampleFor(CapabilitySectionHomeworks, "教学班 作业 <JW ID>", "查看指定教学班作业，每页 30 条", "<JW ID>")}, nil)),
		...
		descriptor(CapabilityUpcomingDeadlines, []string{"upcoming_deadlines", "近期截止"}, CapabilityRequirements{Life: true, OAuth: true, Audience: AudienceGroupReadOnly}, EffectRead, ExposureModel, func(args []string) bool {
