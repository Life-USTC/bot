...
fn validate(req: &GridPayload) -> anyhow::Result<()> {
    ...
    for (name, value) in [
        ...
        if !value.is_finite() || value <= 0.0 {
            return Err(anyhow!("grid {name} must be positive"));
        }
    }
...
fn data_literal(req: &GridPayload) -> String {
    ...
    out.push_str(&format!("label_width: {}, ", super::fmt_num(req.label_width)));
    out.push_str(&format!("row_height: {}, ", super::fmt_num(req.row_height)));
    out.push_str(&format!("header_height: {}, ", super::fmt_num(req.header_height)));
