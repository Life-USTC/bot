...
#context {
  ...
  let next-width = if data.next_time == none { 0pt } else { 40pt + measure(flow-text(size: 11pt, "下一班 " + data.next_time + " " + data.next_wait)).width }
    ...
    if data.next_time == none {
      ...
      grid(columns: (1fr, calc.min(next-width - 40pt, calc.min(width, max-content-width) / 2)),
        ...
        move(dy: -10pt, align(right, {
          flow-text(size: 11pt, fill: muted, "下一班 " + data.next_time)
          v(7pt)
          flow-text(size: 11pt, fill: accent, data.next_wait)
        })))
      v(18pt)
    }
